...
degree.network <- (
  Matrix::rowSums(string.network.binary) +
    Matrix::colSums(string.network.binary)
) / 2
...
```{r, warning=FALSE}
qplot(
  degree.network[degree.network <= quantile(degree.network, probs = .99999)],
...
ydata.raw <- colData(brca.primary.solid.tumor) |>
  as.data.frame() |>
...
valid.features <- colnames(xdata.raw)[
  colnames(xdata.raw) %in% names(degree.network[degree.network > 0])
]
xdata.raw <- xdata.raw[
  TCGAbarcode(rownames(xdata.raw)) %in% rownames(ydata.raw), valid.features
]
...
```{r, eval=!is.null(brca)}
set.seed(params$seed)
foldid <- glmSparseNet:::balanced_cv_folds(!!ydata$status)$output
```
...
selected_genes[["Hub"]] <- Filter(
  function(.x) .x != 0,
  coef(cv.hub, s = "lambda.min")[, 1]
) |>
  names() |>
  geneNames() |>
...
glmSparseNet::separate2GroupsCox(
  as.vector(coef(cv.orphan, s = "lambda.min")[, 1]),
  xdata, ydata,
  plot.title = "Full dataset",
  legend.outside = FALSE
)

selected_genes[["Orphan"]] <- Filter(
  function(.x) .x != 0,
  coef(cv.orphan, s = "lambda.min")[, 1]
) |>
  names() |>
  geneNames() |>
  magrittr::extract2("external_gene_name")
...
selected_genes[["GLMnet"]] <- Filter(
  function(.x) .x != 0,
  coef(cv.glm, s = "lambda.min")[, 1]
) |>
  names() |>
  geneNames() |>
  magrittr::extract2("external_gene_name")
...
```{r, echo=FALSE, warning=FALSE, eval=!is.null(brca)}
venn_plot <- venn.diagram(selected_genes,
  NULL,
  fill = c(
    my_colors(5), my_colors(3),
    my_colors(4)
  ),
  alpha = c(0.3, 0.3, .3),
  cex = 2,
...
melt(selected_genes) |>
  ...
  geom_point(
    ...
    shape = my_symbols(3), color = my_colors(3)
  ) +
  theme(
