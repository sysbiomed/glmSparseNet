...
```{r, warning=FALSE}
qplot(
  degree.network[degree.network <= quantile(degree.network,
    probs = .99999
  )],
...
selected.genes[["Hub"]] <- Filter(\(.x) .x != 0, coef(cv.hub, s = "lambda.min")[, 1]) |>
  names() |>
  geneNames() |>
...
selected.genes[["Orphan"]] <- Filter(\(.x) .x != 0, coef(cv.orphan, s = "lambda.min")[, 1]) |>
  names() |>
  geneNames() |>
  magrittr::extract2("external_gene_name")
```
...
selected.genes[["GLMnet"]] <- Filter(\(.x) .x != 0, coef(cv.glm, s = "lambda.min")[, 1]) |>
  names() |>
  geneNames() |>
  magrittr::extract2("external_gene_name")
```
