...
params:
  project: 'skcm' # skcm prad brca
  tissue: 'metastatic' # primary.solid.tumor metastatic solid.tissue.normal
  coding.genes: !r TRUE
  ...
  target.vars: !r list(classic.cv = 28) # list(classic.cv = 28, degree.cv = 42, orphan.cv = 70)
  #
  seed: !r 1985
  mc.cores: !r 12
  #
...
my.data <- run.cache(prepare.tcga.survival.data, params$project, params$tissue, handle.duplicates = params$handle.duplicates, input.type = 'mutation',
                                #
...
if (params$degree.type == 'oldie') {
  ...
  my.env <- new.env()
  load('../../network.cox-big.files/saves/degree-string.RData', envir = my.env)
...
my.glmnet <- function(prefix, xdata.train, ydata.train, xdata.ix, xdata.train.digest, penalty.factor, target, lambda.min.ratio = .001, show.messages = FALSE, force.recalc = FALSE) {
  ...
  } else if (any(new.model$df > target)) {
...
#outer.result <- lapply(seq_along(glmnet.params), function(ix) {
  ...
  #
  suppressWarnings(
  result  <- my.glmnet(ix.cache, xdata.train, ydata.train, xdata.ix, xdata.train.digest, el$penalty, el$target, show.messages = T)
  )
  
  if (sum(result$coef != 0) < (target - 5)) {
    suppressWarnings(
      result <- my.glmnet(ix.cache, xdata.train, ydata.train, xdata.ix, xdata.train.digest, el$penalty, el$target, lambda.min.ratio = .00001, show.messages = F)
    )
  }
  #
...
my.draw.kaplan <- function(coef.l, plot.title, xdata, ydata, xdata.ix) { 
  return(draw.kaplan(coef.l, xdata[,xdata.ix], ydata, 
...
for (ix.name in names(coefs)) {
  km.train[[ix.name]] <- my.draw.kaplan(list(ix.name = coefs[[ix.name]]), plot.title = 'Train set', xdata.train, ydata.train, xdata.ix)
  km.test[[ix.name]]  <- my.draw.kaplan(list(ix.name = coefs[[ix.name]]), plot.title = 'Test set', xdata.test, ydata.test, xdata.ix)
}
```
...
c.index.fun <- function(all.pairs, fitted.risk, ydata, n.cores = params$mc.cores) {
...
```{r call.results.decl, echo=FALSE, eval=TRUE}
...
call.results <- function(seed, xdata, ydata, train.perc, subset, penalty.factor.degree.new, penalty.factor.degree.old, penalty.factor.orphan) {
  ...
  #outer.result <- lapply(seq_along(glmnet.params), function(ix) {
    ...
    #
    suppressWarnings(
    result  <- my.glmnet(ix.cache, xdata.train, ydata.train, xdata.ix, xdata.train.digest, el$penalty, el$target, show.messages = F)
    )
  ...
  for (ix.name in names(coefs)) {
    km.train[[ix.name]] <- my.draw.kaplan(list(ix.name = coefs[[ix.name]]), plot.title = 'Train set', xdata.train, ydata.train, xdata.ix)$pvalue
    km.test[[ix.name]]  <- my.draw.kaplan(list(ix.name = coefs[[ix.name]]), plot.title = 'Test set', xdata.test, ydata.test, xdata.ix)$pvalue
    #
    c.index.train[[ix.name]] <- fit.risk(coefs[[ix.name]], xdata.train, ydata.train, ix.name, n.cores = 1, show.message = FALSE)
    c.index.test[[ix.name]]  <- fit.risk(coefs[[ix.name]], xdata.test, ydata.test, ix.name, n.cores = 1, show.message = FALSE)
  }
...
if (params$train == 1) {
  ...
  #}, mc.cores = 1, mc.allow.recursive = FALSE)
  }, mc.cores = min( params$mc.cores), mc.allow.recursive = FALSE)
...
```{r}
vv <- Venn(list(GLMNET = consensus.external$glmnet.classic.cv.28$external_gene_name,
           Degree = consensus.external$degree_new.classic.cv.28$external_gene_name,
           Orphan = consensus.external$orphan.classic.cv.28$external_gene_name)) %>% compute.Venn
gp <- VennThemes(vv)
plot(vv, gp = gp)
```

```{r}
sort(consensus.external$glmnet.classic.cv.28$external_gene_name)
```
