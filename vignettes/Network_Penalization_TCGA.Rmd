...
my.data <- run.cache(prepare.tcga.survival.data, 'brca', 'primary.solid.tumor', 'keep_first',
                                #
...
if (params$degree.type == 'oldie') {
  ...
  degree <- degree.cor.inv.weighted(xdata.raw,
...
#
penalty.factor.degree.log <- penalty.factor.degree <- 1 / norm.orig.penalty.factor

inf.ix <- is.infinite(penalty.factor.degree)
#
log.penal                          <- log(penalty.factor.degree[!inf.ix]) + 1
penalty.factor.degree.log[!inf.ix] <- log.penal
penalty.factor.degree.log[inf.ix]  <- max(log.penal) + 1
#
non.log                       <- penalty.factor.degree[!inf.ix]
penalty.factor.degree[inf.ix] <- max(non.log) + 1
...
degree.factor.freq.plot <- ggplot(melt(my.df[,c('ix', 'penalty.factor.degree')], id.vars = c('ix'))) +
  geom_freqpoly(aes(value, color = variable), alpha = 0.5, bins = 200) +
  theme_minimal() + theme(legend.position = 'none') + scale_x_continuous(trans = 'log10', breaks = c(1, 2, 5, 10, 20, 30, 40, 50)) +
  ggtitle('DegreeCox (old) penalty', subtitle = sprintf('Number of total variables: %d', ncol(xdata.train))) +
  xlab('Penalty (log10 scale)') +  scale_y_continuous(trans = 'log10', breaks = c(1, 10, 100, 1000, 10000,  30000)) +
  ylab('Frequency (log10 scale)')

degree.new.factor.freq.plot <- ggplot(melt(my.df[,c('ix', 'penalty.factor.degree.new')], id.vars = c('ix'))) +
  geom_freqpoly(aes(value, color = variable), alpha = 0.5, bins = 200) +
  theme_minimal() + theme(legend.position = 'none') + scale_x_continuous(trans = 'log10', breaks = c(1, 2, 5, 10, 20, 30, 40, 50)) +
  ggtitle('DegreeCox (heuristic) penalty', subtitle = sprintf('Number of total variables: %d', ncol(xdata.train))) +
  xlab('Penalty (log10 scale)') +  scale_y_continuous(trans = 'log10', breaks = c(1, 10, 100, 1000, 10000,  30000)) +
  ylab('Frequency (log10 scale)')

degree.log.factor.freq.plot <- ggplot(melt(my.df[,c('ix', 'penalty.factor.degree.log')], id.vars = c('ix'))) +
  geom_freqpoly(aes(value, color = variable), alpha = 0.5, bins = 200) +
  theme_minimal() + theme(legend.position = 'none') + scale_x_continuous(trans = 'log10', breaks = c(1, 2, 5, 10, 20, 30, 40, 50)) +
  ggtitle('DegreeCox (log) penalty', subtitle = sprintf('Number of total variables: %d', ncol(xdata.train))) +
  xlab('Penalty (log10 scale)') +  scale_y_continuous(trans = 'log10', breaks = c(1, 10, 100, 1000, 10000,  30000)) +
  ylab('Frequency (log10 scale)')

suppressWarnings({
  multiplot(original.freq.plot, degree.factor.freq.plot, orphan.freq.plot, degree.log.factor.freq.plot, degree.new.factor.freq.plot)
})
```
...
transf.d <- melt.data.frame(my.df[,c('ix', 
                                     'original', 
                                     'penalty.factor.degree',
                                     'penalty.factor.orphan', 
                                     'penalty.factor.degree.new', 
                                     'penalty.factor.degree.log')], id.vars = c('ix'), variable_name = 'Type')
levels(transf.d$Type) <- c('Original', 'Degree (old)', 'Orphan', 'Degree (heuristic)', 'Degree (log(old))')
...
for (target.name in names(params$target.vars)) {
  ...
  glmnet.params[[ix + 2]] <- list(penalty = penalty.factor.degree.new, name = 'degree.new', target = target, target.name = target.name)
  glmnet.params[[ix + 3]] <- list(penalty = penalty.factor.degree.log, name = 'degree.log', target = target, target.name = target.name)
  glmnet.params[[ix + 4]] <- list(penalty = penalty.factor.orphan, name = 'orphan', target = target, target.name = target.name)
  ix <- ix + 5
}
