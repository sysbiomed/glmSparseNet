...
my.glmnet <- function(prefix, xdata.train, ydata.train, xdata.ix, xdata.train.digest, penalty.factor, target, lambda.min.ratio = .001, show.messages = FALSE) {
...
my.draw.kaplan <- function(coef.l, plot.title, test.set = F) { 
  if (test.set) {
    return(draw.kaplan(coef.l, xdata.test[,xdata.ix], ydata.test, plot.title = plot.title, legend.outside = F, ylim = c(0,1))) 
  } else {
    draw.kaplan(coef.l, xdata.train[,xdata.ix], ydata.train, plot.title = plot.title, legend.outside = F, ylim = c(0,1))
  }
}
...
for (ix.name in names(coefs)) {
  km.train[[ix.name]] <- my.draw.kaplan(list(ix.name = coefs[[ix.name]]), plot.title = 'Train set')
  km.test[[ix.name]]  <- my.draw.kaplan(list(ix.name = coefs[[ix.name]]), plot.title = 'Test set', test.set = T)
}
```
...
fit.risk <- function(coef.v, xdata, ydata, model.name, show.message = FALSE, n.cores = params$mc.cores) {
  ...
  c.res <- run.cache(survcomp::concordance.index,fitted.risk, ydata[,'time'], ydata[,'status'], method="noether", show.message = F)
  if (show.message) flog.info(' * %s: %f (se = %f | pvalue = %f)', model.name, c.res$c.index, c.res$se, c.res$pvalue)
...
call.results <- function(seed, xdata, ydata, train.perc, subset, penalty.factor.degree.new, penalty.factor.degree.old, penalty.factor.orphan) {
  ...
  for (ix.name in names(coefs)) {
    km.train[[ix.name]] <- my.draw.kaplan(list(ix.name = coefs[[ix.name]]), plot.title = 'Train set')$pvalue
    km.test[[ix.name]]  <- my.draw.kaplan(list(ix.name = coefs[[ix.name]]), plot.title = 'Test set', test.set = T)$pvalue
    #
    c.index.train[[ix.name]] <- fit.risk(coefs[[ix.name]], xdata.train, ydata.train, ix.name, n.cores = 1, show.message = FALSE)
    c.index.test[[ix.name]]  <- fit.risk(coefs[[ix.name]], xdata.train, ydata.train, ix.name, n.cores = 1, show.message = FALSE)
  }
...
if (params$train == 1) {
  ...
  rm(xdata.train, xdata.test, ydata.train, ydata.test)
  set.seed(params$seed)
