...
params:
  ...
  degree.cutoff: !r 0
  #
  degree.type: "covariance" # covariance, correlation
  #
  normalize: 'max'
...
#run.me <- list(
#  skcm.metastatic          = list(project = 'skcm', tissue = 'metastatic',          degree.correlation = 'pearson', degree.unweighted = FALSE, degree.cutoff = 0.5),
#  skcm.primary.solid.tumor = list(project = 'skcm', tissue = 'primary.solid.tumor', degree.correlation = 'pearson', degree.unweighted = FALSE, degree.cutoff = 0.5),
  #
#  brca.solid.tissue.normal = list(project = 'brca', tissue = 'solid.tissue.normal', degree.correlation = 'pearson', degree.unweighted = FALSE, degree.cutoff = 0.5),
#  brca.primary.solid.tumor = list(project = 'brca', tissue = 'primary.solid.tumor', degree.correlation = 'pearson', degree.unweighted = FALSE, degree.cutoff = 0.1),
#  brca.primary.solid.tumor = list(project = 'brca', tissue = 'primary.solid.tumor', degree.correlation = 'pearson', degree.unweighted = FALSE, degree.cutoff = 0.2),
#  brca.primary.solid.tumor = list(project = 'brca', tissue = 'primary.solid.tumor', degree.correlation = 'pearson', degree.unweighted = FALSE, degree.cutoff = 0.3),
#  brca.primary.solid.tumor = list(project = 'brca', tissue = 'primary.solid.tumor', degree.correlation = 'pearson', degree.unweighted = FALSE, degree.cutoff = 0.4),
#  brca.primary.solid.tumor = list(project = 'brca', tissue = 'primary.solid.tumor', degree.correlation = 'pearson', degree.unweighted = FALSE, degree.cutoff = 0.5),
#  brca.primary.solid.tumor = list(project = 'brca', tissue = 'primary.solid.tumor', degree.correlation = 'pearson', degree.unweighted = FALSE, degree.cutoff = 0.6),
#  brca.primary.solid.tumor = list(project = 'brca', tissue = 'primary.solid.tumor', degree.correlation = 'pearson', degree.unweighted = FALSE, degree.cutoff = 0.7),
#  brca.primary.solid.tumor = list(project = 'brca', tissue = 'primary.solid.tumor', degree.correlation = 'pearson', degree.unweighted = FALSE, degree.cutoff = 0.8),
#  brca.primary.solid.tumor = list(project = 'brca', tissue = 'primary.solid.tumor', degree.correlation = 'pearson', degree.unweighted = FALSE, degree.cutoff = 0.9),
#  brca.primary.solid.tumor = list(project = 'brca', tissue = 'primary.solid.tumor', degree.correlation = 'pearson', degree.unweighted = FALSE, degree.cutoff = 1),
  #
  #prad.solid.tissue.normal = list(project = 'prad', tissue = 'solid.tissue.normal', degree.correlation = 'pearson', degree.unweighted = FALSE, degree.cutoff = 0.5),
  #prad.primary.solid.tumor = list(project = 'prad', tissue = 'primary.solid.tumor', degree.correlation = 'pearson', degree.unweighted = FALSE, degree.cutoff = 0.5))
# 
run.me <- list()

for(ix in seq(0,1,0.05)) {
  run.me[[paste0("brca.primary.solid.tumor-", sprintf("%.2f", ix))]] <- list(project = 'brca', tissue = 'primary.solid.tumor', degree.correlation = 'pearson', degree.unweighted = FALSE, degree.cutoff = ix)
}

for (ix.name in names(run.me)) {
  el <- run.me[[ix.name]]
  un <- if(el$degree.unweighted) { 'unweighted' } else { 'weighted' }
  out.name <- sprintf('%s-%s-%s-%s-%.2f.html', el$project, el$tissue, el$degree.correlation, un, el$degree.cutoff)
  tryCatch({
    rmarkdown::render('Network_Penalization_TCGA.Rmd', output_file = out.name, params = el)    
  }, error = function(str) {
    flog.error('Error when rendering Rmd: %s', str$message)
  })
}
```
...
```{r, eval=FALSE, include=FALSE}
# testing different methods of calculating degree
bb <- t(fpkm.per.tissue$primary.solid.tumor)
sd.bb <- apply(bb, 2, sd)

bb.orig <- bb[,sd.bb != 0]

bb.orig.norm    <- t(apply(bb.orig, 1, function(row) { row / max(row) }))
sd.bb.orig.norm <- apply(bb.orig.norm, 2, sd)
bb.orig.norm    <- bb.orig.norm[, sd.bb.orig.norm != 0]

bb.trim    <- bb.orig[rownames(xdata),]
sd.bb.trim <- apply(bb.trim, 2, sd)
bb.trim    <- bb.trim[,sd.bb.trim != 0]

bb.trim.norm    <- t(apply(bb.trim, 1, function(row) { row / max(row) }))
sd.bb.trim.norm <- apply(bb.trim.norm, 2, sd)
bb.trim.norm    <- bb.trim.norm[, sd.bb.trim.norm != 0]

ix.d <- 2
pp <- list()
ss <- list()
pp$orig <- as.vector(sapply((1:ncol(bb.orig))[-ix.d], function(ix) {abs(cor(bb.orig[,ix.d],bb.orig[,ix], method = 'pearson')) })))
ss$orig <- as.vector(sapply((1:ncol(bb.orig))[-ix.d], function(ix) {abs(cor(bb.orig[,ix.d],bb.orig[,ix], method = 'spearman')) }))
pp$orig.norm <- as.vector(sapply((1:ncol(bb.orig.norm))[-ix.d], function(ix) {abs(cor(bb.orig.norm[,ix.d],bb.orig.norm[,ix], method = 'pearson')) }))
ss$orig.norm <- as.vector(sapply((1:ncol(bb.orig.norm))[-ix.d], function(ix) {abs(cor(bb.orig.norm[,ix.d],bb.orig.norm[,ix], method = 'spearman')) }))
pp$trim <- as.vector(sapply((1:ncol(bb.trim))[-ix.d], function(ix) {abs(cor(bb.trim[,ix.d],bb.trim[,ix], method = 'pearson')) }))
ss$trim <- as.vector(sapply((1:ncol(bb.trim))[-ix.d], function(ix) {abs(cor(bb.trim[,ix.d],bb.trim[,ix], method = 'spearman')) }))
pp$trim.norm <- as.vector(sapply((1:ncol(bb.trim.norm))[-ix.d], function(ix) {abs(cor(bb.trim.norm[,ix.d],bb.trim.norm[,ix], method = 'pearson')) }))
ss$trim.norm <- as.vector(sapply((1:ncol(bb.trim.norm))[-ix.d], function(ix) {abs(cor(bb.trim.norm[,ix.d],bb.trim.norm[,ix], method = 'spearman')) }))

for (ix.name in names(pp)) {
  flog.info('%s', ix.name)
  flog.info('   pearson: %g', sum(pp[[ix.name]][pp[[ix.name]] > 0.5]))
  flog.info('  spearman: %g', sum(ss[[ix.name]][ss[[ix.name]] > 0.5]))
}
```

```{r data_normalization, echo=FALSE}
xdata.raw <- apply(fpkm.per.tissue[[params$tissue]], 1, function(row) {
  if (max(abs(row)) == 0) {
    return(row)
  }
...
if (params$degree.type == 'oldie') {
  ...
  degree <- degree.weighted(xdata.raw, 
  ...
  degree <- degree.covariance / max(degree.covariance) * 43799
}

if (params$handle.duplicates == 'keep_first') {
  xdata.raw <- xdata.raw[!duplicated(strtrim(rownames(xdata.raw), 12)),]
}
xdata <- xdata.raw
#
#if (params$normalize == 'max') {
#  xdata <- matrix(unlist(parallel::mclapply(seq(ncol(xdata.raw)), function(ix) { 
#    return(xdata.raw[,ix] / max(abs(xdata.raw[,ix])))
#  }, mc.cores = 16)), ncol = ncol(xdata.raw), byrow = FALSE)  
#} else if (params$normalize == 'scale_center') {
#  xdata <- matrix(unlist(parallel::mclapply(seq(ncol(xdata.raw)), function(ix) { 
#    return(scale(xdata.raw[,ix], center = TRUE, scale = TRUE))
#  }, mc.cores = 16)), ncol = ncol(xdata.raw), byrow = FALSE)  
#} else if (params$normalize == 'scale') {
#  xdata <- matrix(unlist(parallel::mclapply(seq(ncol(xdata.raw)), function(ix) { 
#    return(scale(xdata.raw[,ix], center = FALSE, scale = TRUE))
#  }, mc.cores = 16)), ncol = ncol(xdata.raw), byrow = FALSE)  
#}
#
xdata.digest.cache <- verissimo::digest.cache(xdata)
#
#colnames(xdata) <- colnames(xdata.raw)
#rownames(xdata) <- rownames(xdata.raw)
#
```
...
```{r clinical_data, echo=FALSE}
ix.clinical <- !is.na(clinical[[params$tissue]]$bcr_patient_barcode)

ydata <- data.frame(time = clinical[[params$tissue]]$surv_event_time,
                    status = clinical[[params$tissue]]$vital_status)[ix.clinical,]

rownames(ydata) <- clinical[[params$tissue]]$bcr_patient_barcode[ix.clinical]
...
if (length(strtrim(rownames(xdata), 12)) != length(unique(strtrim(rownames(xdata), 12)))) {
  flog.warn('There are multiple samples for the same individual.. using strategy: \'%s\'', params$handle.duplicates)
  #
...
```{r}
age.birth <- clinical$all[strtrim(rownames(ydata), 12), 'days_to_birth']
names(age.birth) <- rownames(ydata)
age.birth <- age.birth * -1
age.birth <- age.birth[!is.nan(age.birth)]
age.birth <- age.birth[!is.na(age.birth)]

age.birth.model <- coxph(Surv(ydata[names(age.birth),1], ydata[names(age.birth),2]) ~ age.birth)
age.birth.model
```
...
Ver KM de month e year of diagnostic

```{r}
#clinical$all$month_of_form_completion
```
...
trans.fun <- function(x, 
                      ...
                      b = -1, 
                      g = 1) { 
  return(a + exp(-b * (exp(x) + g)) - 1)
}
#trans.fun <- function(x) { .2 + exp(x) - 1 }
```
...
my.glmnet <- function(prefix, penalty.factor, lambda.min.ratio = .001, show.messages = FALSE) {
...
if (sum(coefs$glmnet != 0) < (params$target.vars - 5)) {
  result$glmnet <- my.glmnet('glmnet_models', rep(1, ncol(xdata.train)), lambda.min.ratio = .0001, show.messages = FALSE)
  models$glmnet  <- result$glmnet$model
  lambdas$glmnet <- result$glmnet$lambda
  coefs$glmnet   <- result$glmnet$coef
}
...
if (sum(coefs$degree != 0) < (params$target.vars - 5)) {
  result$degree <- my.glmnet('degree_models', penalty.factor.degree, lambda.min.ratio = .0001, show.messages = FALSE)
  models$degree  <- result$degree$model
  lambdas$degree <- result$degree$lambda
  coefs$degree   <- result$degree$coef
}
...
if (sum(coefs$orphan != 0) < (params$target.vars - 5)) {
  result$orphan <- my.glmnet('orphan_models', penalty.factor.orphan, lambda.min.ratio = .0001, show.messages = FALSE)
  models$orphan  <- result$orphan$model
  lambdas$orphan <- result$orphan$lambda
  coefs$orphan   <- result$orphan$coef
}
...
fitted.network <- build.fit.df(list(Classic = list(model = models$glmnet, target = lambdas$glmnet),
...
km.test$glmnet  <- my.draw.kaplan(list(GLMNET = coefs$glmnet), filename = 'Test set', test.set = T)
km.test$degree  <- my.draw.kaplan(list(Degree = coefs$degree), filename = 'Test set', test.set = T)
km.test$orphan  <- my.draw.kaplan(list(Orphan = coefs$orphan), filename = 'Test set', test.set = T)
```
...
# Classic model
c.index.train$glmnet <- fit.risk(coefs$glmnet, xdata.train, ydata.train, '   GLMNET',  show.message = TRUE)
# DegreeCox
c.index.train$degree <- fit.risk(coefs$degree, xdata.train, ydata.train, 'DegreeCox', show.message = TRUE)
# OrphanCox
c.index.train$orphan <- fit.risk(coefs$orphan, xdata.train, ydata.train, 'OrphanCox', show.message = TRUE)
...
# Classic model
c.index.test$glmnet <- fit.risk(coefs$glmnet, xdata.test, ydata.test, '   GLMNET', show.message = TRUE)
# DegreeCox
c.index.test$degree <- fit.risk(coefs$degree, xdata.test, ydata.test, 'DegreeCox', show.message = TRUE)
# OrphanCox
c.index.test$orphan <- fit.risk(coefs$orphan, xdata.test, ydata.test, 'OrphanCox', show.message = TRUE)
...
this.name <- function(this.coef) {
  tryCatch({
    ...
    mart    <- biomaRt::useMart(marts$biomart[index], 'hsapiens_gene_ensembl')
...
```{r save_genes_rdata, include=FALSE}
...
```{r call.results.decl, echo=FALSE}
...
xdata.df <- data.frame(xdata.train[,sample(seq_along(xdata.ix), size = 100)])
xdata.df <- cbind(time = ydata.train$time, event = ydata.train$status, xdata.df)
n <- colnames(xdata.df)
f <- as.formula(paste("survival::Surv(time, event) ~", paste(n[!n %in% c('time', 'event')], collapse = " + ")))
my.call <- call('ElasticNet', formula = f, data = xdata.df,
           nfolds           = 5,
           #
           family           ='cox', 
           alpha            = params$alpha, 
           ...
           standardize      = F)
en.respo <- list(call = my.call, enet = models$glmnet, Lambda = lambdas$glmnet)
class(en.respo) <- 'ElasticNet'
Brier(en.respo)
