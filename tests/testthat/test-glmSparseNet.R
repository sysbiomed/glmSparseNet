...
test_that("glmSparseNet: simple call with MultiAssayExperiment", {
  ...
  xdata$surv_event_time <- array(NA_integer_, nrow(colData(xdata)))
  ...
  xdata_valid <- xdata[, rownames(colData(xdata))[valid_ix]]
  ydata_valid <- colData(xdata_valid)[, c("surv_event_time", "vital_status")]
