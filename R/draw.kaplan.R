...
separate2GroupsCox <- function(chosen.btas, xdata, ydata,
  ...
  if (!inherits(chosen.btas, 'list')) {
    stop(paste0('chosen.btas argument must be a list or vector. ',
                'See documentation ?separate2GroupsCox'))
  } else if (!inherits(xdata, 'matrix')) {
    stop(paste0('xdata argument must be a matrix, data.frame or vector ',
                'See documentation ?separate2GroupsCox'))
  } else if (!inherits(ydata, 'data.frame')) {
    stop(paste0('ydata argument must be a data.frame. ',
                'See documentation ?separate2GroupsCox'))
  }
  ...
  if (nrow(xdata) != nrow(ydata)) {
    ...
    stop(sprintf(paste0('All or some of the chosen.btas (%s) have different ',
  ...
  prognostic.index <- tryCatch(xdata %*% chosen.btas.mat, error = function(err){
    ...
    cat('      xdata (nrow)x(ncol) =', sprintf('%dx%d', nrow(xdata),
                                               ncol(xdata)), '\n')
    cat('chosen.btas (nrow)x(ncol) =', sprintf('%dx%d', nrow(chosen.btas.mat),
  ...
  if (length(levels(prognostic.index.df$group)) == 1) {
    stop(paste0('separate2GroupsCox(): There is only one group, cannot create ',
  ...
  if (length(chosen.btas) > 1) {
    ...
    p1 <- p1 +
      ggplot2::scale_colour_manual(values = c(loose.rock::my.colors()[col.ix]))
