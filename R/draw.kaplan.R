...
separate2GroupsCox <- function(chosen.btas, xdata, ydata,
                               ...
                               xlim = NULL, ylim = NULL, expand.yzero = FALSE, legend.outside = FALSE) {
  ...
  # convert between compatible formats
  if (inherits(xdata, 'data.frame') || inherits(xdata, 'numeric') || inherits(xdata, 'matrix')) {
  ...
  if (!inherits(chosen.btas, 'list')) {
    ...
    stop('xdata argument must be a matrix, data.frame or vector See documentation ?separate2GroupsCox')
  } else if (!inherits(ydata, 'data.frame')) {
    stop('ydata argument must be a data.frame. See documentation ?separate2GroupsCox')
  }
