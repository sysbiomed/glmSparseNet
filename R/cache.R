    ...
    vapply(
      ...
      function(ix) {
        if (is.null(attributes(methodsFound[[ix]])$srcref)) {
          ...
          return(.digestCache(toString(attributes(methodsFound[[ix]])$srcref)))
        }
      },
      character(1L)
    )
    ...
    result <- tryCatch(
      {
        ...
        if (
          ...
          myMsg <- paste0(
            "Cache was created at ", .POSIXct(tmpEnv$epochMilliseconds / 1000)
          )
          if (!is.null(tmpEnv$spec) && !is.na(tmpEnv$spec["version"])) {
            message(myMsg, " using glmSparseNet v", tmpEnv$spec["version"])
          } else {
            message(myMsg, " using glmSparseNet before v1.0.16 or before")
          }
        }
        tmpEnv$result
      },
